...
// Consultar el saldo del usuario en la moneda nativa de Polygon (POL)
async function consultarSaldoPolygon() {
    if (typeof window.ethereum === 'undefined') {
        alert('MetaMask no está instalado. Por favor, instálalo para usar esta función.');
        return;
    }
    ...
    try {
        ...
        const polBalance = parseFloat(balance) / 1e18;
        alert('Saldo: ' + polBalance.toFixed(6) + ' POL');
    } catch (error) {
...
document.getElementById('consultarSaldo').addEventListener('click', consultarSaldoPolygon); // Cambiar a consultarSaldoPolygon
